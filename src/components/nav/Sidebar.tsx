...
export function Sidebar() {
    ...
    <>
      <div className="flex h-[88px] shrink-0 items-center gap-3 border-b border-sidebarBorder px-5">
        <Link
          ...
          className="gradient-primary flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-lg font-black text-black shadow-elegant"
        >
          J
        </Link>
        <div className="min-w-0">
          <p className="label-caps truncate text-sm text-ink">SECRETÁRIA</p>
          <p className="mt-0.5 truncate text-[9px] font-medium tracking-[0.14em] text-ink-faint">
            ORGANIZAÇÃO PESSOAL
          </p>
        </div>
        <button
          ...
          return (
            <Link
              ...
              className={[
                ...
                active
                  ? 'gradient-soft border-primary/30 text-primary'
  ...
  return (
    <>
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[270px] flex-col border-r border-sidebarBorder bg-sidebar lg:flex">
        {menu}
      </aside>
      ...
      {open && (
        <>
          ...
          />
          <aside className="fixed inset-y-0 left-0 z-50 flex w-[min(86vw,320px)] flex-col border-r border-sidebarBorder bg-sidebar shadow-2xl lg:hidden">
            {menu}
          </aside>
        </>
      )}
