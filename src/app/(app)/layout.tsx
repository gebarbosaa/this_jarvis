  ...
  return (
    <div className="min-h-dvh bg-paper">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[252px] border-r border-sidebarBorder bg-sidebar lg:flex lg:flex-col">
        <div className="flex h-[88px] items-center gap-3 border-b border-sidebarBorder px-5">
          <span className="gradient-primary flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-lg font-black text-black shadow-elegant">
            J
          </span>
          <div className="min-w-0">
            <p className="label-caps truncate text-sm text-ink">SECRETÁRIA</p>
            <p className="mt-0.5 text-[9px] font-medium tracking-[0.14em] text-ink-faint">ORGANIZAÇÃO PESSOAL</p>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-5">
          ...
          {NAV.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="group flex items-center gap-3 rounded-xl border border-transparent px-3 py-2.5 text-[10px] font-bold tracking-[0.07em] text-ink-soft transition-all hover:border-sidebarBorder hover:bg-secondary hover:text-primary"
            >
              <span className="flex h-6 w-6 items-center justify-center rounded-md bg-secondary/60 text-[9px] text-ink-faint transition-colors group-hover:bg-primary/10 group-hover:text-primary">•</span>
              {item.label}
            </a>
          ))}
        </nav>
