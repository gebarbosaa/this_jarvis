...
export async function saveRoutineSettings(formData:FormData){
  ...
  await supabase.from('routine_settings').upsert({user_id:user.id,day_start:String(formData.get('day_start')||'08:00'),day_end:String(formData.get('day_end')||'22:00'),updated_at:new Date().toISOString()});
